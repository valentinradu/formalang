"""Build the Formalang logo in Blender and render to logo.png.

Run headless from project root:
    blender --background --python website/build_logo.py

Output: website/logo.png (1024x1024 PNG with transparent background)
"""
...
LAYER_H = 0.30           # bottom layer (mauve) — uniform thickness
CORNER_RADIUS = 0.32     # vertical-corner roundness (the "soft toy" base corners)
CORNER_SEG = 16          # arc segments per corner

# Top gold layer becomes a wedge: thin at -Y (below the text baseline) so the
...
# Debossed text
TEXT_SIZE = 1.30
TEXT_FONT = "fonts/Hack-Bold.ttf"  # MIT-licensed mono font (Bold = "medium" feel)
# "fv" is centered on the top face; the dot sits to the left, ignored for
# centering so it doesn't pull the lettermark off-axis.
FV_X = 0.0
DOT_X = -0.65

# Toon palette: (shadow, mid, highlight) — Catppuccin Mocha mid-tones, with
# darker shadow and brighter highlight to give the cel shader range.
MAUVE_SHADOW = "#3a1f6a"
MAUVE_MID = "#9c70d8"
MAUVE_LIGHT = "#c69aec"

YELLOW_SHADOW = "#a78a32"
YELLOW_MID = "#eccf78"
YELLOW_LIGHT = "#f5dfa0"
...
def make_toon_material(name, hex_shadow, hex_mid, hex_light,
    ...
    # Highlight stop pushed earlier (0.70) so the lit surface lifts into the
    # bright band more often.
...
def make_gear_cutter(name, teeth, r_outer, r_inner, height, location,
                     tip_fraction=0.22, base_fraction=0.50):
    """Build a gear-shaped prism with tapered teeth (narrow tip, wide base).

    `tip_fraction`/`base_fraction` are the angular share of each tooth slot
    occupied by the outer-arc tip and the inner-radius base; base > tip ⇒
    teeth taper inward as they extend outward.
    """
    mesh = bpy.data.meshes.new(name + "_mesh")
    bm = bmesh.new()

    angle_per_tooth = 2 * math.pi / teeth
    tip_half = angle_per_tooth * tip_fraction / 2
    base_half = angle_per_tooth * base_fraction / 2

    verts_2d = []
    for k in range(teeth):
        center = k * angle_per_tooth
        a_tip = center - tip_half
        b_tip = center + tip_half
        b_base = center + base_half
        c_base = (k + 1) * angle_per_tooth - base_half
        verts_2d.append((r_outer * math.cos(a_tip), r_outer * math.sin(a_tip)))
        verts_2d.append((r_outer * math.cos(b_tip), r_outer * math.sin(b_tip)))
        verts_2d.append((r_inner * math.cos(b_base), r_inner * math.sin(b_base)))
        verts_2d.append((r_inner * math.cos(c_base), r_inner * math.sin(c_base)))

    bm_verts = [bm.verts.new((x, y, 0.0)) for (x, y) in verts_2d]
...
def add_boolean(obj, target, name="Boolean", solver="EXACT"):
    ...
    mod.solver = solver
    mod.object = target
    # Tolerate non-manifold / self-intersecting cutters (e.g., text-to-mesh)
...
def make_cavity_material(name, hex_dark, hex_light, ao_distance=1.4):
    ...
    # Square AO so occluded areas (bowl center) drop sharply
...
mat_mauve = make_toon_material("Mauve", MAUVE_SHADOW, MAUVE_MID, MAUVE_LIGHT)
mat_yellow = make_toon_material(
    "Yellow", YELLOW_SHADOW, YELLOW_MID, YELLOW_LIGHT,
    spec_intensity=0.45, gloss_roughness=0.28,
)
# Cavity uses the toon shader (so the curved bowl gets directional depth shading)
...
# form a ramp (thin at -X, thick at +X).
...
# Tilt the top-face vertices: tilt along Y so the ramp slopes toward the
# bottom of the text (low at -Y, high at +Y).
...
# Text cutters: "." and "fv" placed as two separate objects so the f/v gap
# stays natural while the dot can be pulled in tight against the f. Both
# cutters are tilted by the same ramp angle so the deboss tracks the slope
# at constant depth, not constant world-Z.
...
fv_cutter = make_text_cutter("FvCutter", "fv", FV_X)

# Two boolean cuts on top — debosses the dot and "fv" separately
add_boolean(top, dot_cutter, name="DotBool")
add_boolean(top, fv_cutter, name="FvBool")

# Bevel after the booleans so the cavity rim (text edges) gets softened.
# ANGLE limit catches both the cavity rims and the top-face perimeter.
...
# Position camera along its view direction so it points at the box center
...
# ---------- lighting ----------
# Key sun light: upper-back, tilted toward the front so it grazes the ramp
# perpendicular-ish (highlight lands on the slope itself, not the front face)
bpy.ops.object.light_add(type="SUN", location=(0.0, 4.0, 5.0))
key = bpy.context.active_object
key.data.energy = 1.5
key.rotation_euler = (math.radians(40), 0.0, 0.0)

# Counter-key area light: positioned upper-right so its specular reflection
# off the right-edge curve of the slope hits the camera (creates the
# second sheen on the gold's right side, opposing the front-left sheen).
...
# Fill area light from the right
...
# Dim top-down fill: lifts cavity floor/inner walls without shifting outer faces
...
# Soft world ambient (helps the cavity not go pure black)
world = bpy.data.worlds["World"]
world.use_nodes = True
bg = world.node_tree.nodes["Background"]
# Solid background color — Catppuccin Mocha Crust feel, dark but not pure black
bg.inputs["Color"].default_value = hex_to_rgba("#11111b")
bg.inputs["Strength"].default_value = 1.0

# ---------- render ----------
scene = bpy.context.scene

# Pick whichever Eevee identifier this Blender exposes
