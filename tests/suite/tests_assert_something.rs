...
/// Where the test files are. The suite is one binary, so every test
/// file sits in this directory.
const TESTS: &str = "tests/suite";
...
fn no_file_grows_its_count_of_tests_that_assert_nothing() {
    ...
    let entries = std::fs::read_dir(Path::new(TESTS)).expect("tests/suite/ must be readable");
