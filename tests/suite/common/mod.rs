//! Shared test helpers. Not a public API — only consumed by the
//! integration tests in `tests/suite/`.
//!
//! The suite is one binary: `tests/suite/main.rs` declares this module
//! once, and every test file reaches it through `crate::common::`.
...
/// The name that the test harness knows a test by.
///
/// The suite is one binary, so the name of a test carries the module
/// that holds it: `metamorphic::compile_is_deterministic…`. A test that
/// starts itself again as a child process must pass that whole name to
/// `--exact`. `module_path!()` gives `suite::metamorphic`; the crate
/// segment is not part of the name, so this function removes it.
