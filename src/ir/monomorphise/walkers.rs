...
pub(super) fn walk_function_spans_mut(
    f: &mut IrFunction,
    visit: &mut impl FnMut(&mut crate::ir::IrSpan),
) {
    for p in &mut f.params {
        visit(&mut p.span);
        if let Some(d) = &mut p.default {
...
pub(super) fn walk_expr_spans_mut(
    expr: &mut IrExpr,
    visit: &mut impl FnMut(&mut crate::ir::IrSpan),
) {
...
fn walk_expr_spans_mut_inner(expr: &mut IrExpr, visit: &mut impl FnMut(&mut crate::ir::IrSpan)) {
    visit(expr.span_mut());
    match expr {
        IrExpr::Literal { .. }
        | IrExpr::Reference { .. }
