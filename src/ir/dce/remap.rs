...
pub(super) fn remove_unused_definitions(
    ...
    //
    // Walk the impls vector once, build an old-index → new-index
    // table, then retain in lockstep. The table is fed to
    // `apply_impl_index_remap` so every `DispatchKind::Static { impl_id }`
    // call site is rewritten to the new compacted position.
...
fn build_impl_index_remap(module: &IrModule, remap: &IdRemap) -> Vec<Option<usize>> {
